pool.BR1999 <- function (Q, U, dfcom) 
{
  m <- length(Q)
  qbar <- mean(Q)
  ubar <- mean(U)
...
#evaluation function
evaluate.impute.BR <- function(OUT, truth1 = population$ymis1, truth2 = population$ymis2){
  POOL1 <- list(NA)
  POOL.FIN1 <- list(NA)
  POOL2 <- list(NA)
  POOL.FIN2 <- list(NA)
  ...
  for (z in 1: length(OUT)){
    ...
    pool         <- pool.BR1999(Q, U, sum(!is.na(OUT[[z]]$data$ymis1)))
    ...
    #Pool the regular way
    pool         <- pool.BR1999(Q, U, sum(!is.na(OUT[[z]]$data$ymis2)))
    pool$lower   <- pool$qbar - qt(.975, pool$df) * sqrt(pool$t)
    pool$upper   <- pool$qbar + qt(.975, pool$df) * sqrt(pool$t)
